...
from playsound import * #для mp3

clicked = True #можно кликать\нажимать
count = 0 #для счета

playsound = "Led+Zeppelin-The+Immigrant+Song.mp3"
...
def checkWinner():
    ...
    if button1["text"] == "X" and button2["text"] == "X" and button3["text"] == "X":
        ...
        messagebox.showinfo("Game over", "1 pobedil")
        start()
        #done
    ...
    #krestiki done
        ...
        button3.config(bg="lime")# (0) (0) (0) 
        winner = True #победитель известен
        messagebox.showinfo("Game over", "2 pobedil")
        start()
        #done
...
def checkDraw():
    ...
    if winner == False and count == 9: #winner(победителя нет), count(все 9 клеток занято без победителя)
        messagebox.showinfo("Game over", "Два лоха!") #Коробка с инфой
        start()
